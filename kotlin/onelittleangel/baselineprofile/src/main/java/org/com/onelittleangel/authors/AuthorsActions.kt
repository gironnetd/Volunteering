...
fun MacrobenchmarkScope.authorsWaitForContent() {
    ...
    val obj = device.findObject(By.res("authors:feed"))
...
fun MacrobenchmarkScope.forYouSelectAuthors(recheckAuthorsIfChecked: Boolean = false) {
    val authors = device.findObject(By.res("authors:feed"))
...
fun MacrobenchmarkScope.authorsScrollFeedDownUp() {
    val feedList = device.findObject(By.res("authors:feed"))
