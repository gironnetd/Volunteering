...
fun FaithCard(
    ...
    Card(
        ...
        modifier = modifier
            ...
            .clickable { faith.followableTopics?.first()?.let { onFaithClick(it) } }
    ) {
        Column(
            modifier = Modifier.fillMaxWidth().padding(all = 16.dp),
            verticalArrangement = Arrangement.spacedBy(8.dp)
        ) {
            CardTitle(title = faith.name)

            faith.followableTopics?.first()?.topic?.shortDescription?.let {
