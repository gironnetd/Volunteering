  ...
  private class InternalViewPagerListener implements ViewPager.OnPageChangeListener {
    ...
    public void onPageScrollStateChanged(int state) {
      ...
      if(state == ViewPager.SCROLL_STATE_IDLE) {
        if(soundPool != null && soundPool.get() != null) {
          soundPool.get().release();
          soundPool.get().setOnLoadCompleteListener(null);
          soundPool = null;
        }
      }
    }
      ...
      if(motionEvent.getAction() == MotionEvent.ACTION_DOWN){
        for (int i = 0; i < tabStrip.getChildCount(); i++) {
          if (view == tabStrip.getChildAt(i)) {
            ...
            int soundID = soundPool.get().load(OnelittleAngelApplication.instance.getBaseContext(), R.raw.unlock, 1);
