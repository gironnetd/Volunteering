 ...
 */
package com.sc.fr.taoisme.layers.mvp.biography.smarttablayout;
...
class SmartTabStrip extends LinearLayout {
  ...
  private static final int DEFAULT_SELECTED_INDICATOR_COLOR = 0;
  ...
  SmartTabStrip(Context context, AttributeSet attrs) {
    ...
    int indicatorColor = settings.getInt(CardViewNative.DARKERRGB,0);
  ...
  void onViewPagerPageChanged(int position, float positionOffset) {
    ...
    if (Float.floatToRawIntBits(positionOffset) == 0 && lastPosition != selectedPosition) {
  ...
  private SmartTabLayout.TabColorizer getTabColorizer() {
  ...
  private void drawIndicator(Canvas canvas, int left, int right, int height, float thickness,
    ...
    switch (indicatorGravity) {
      ...
      case GRAVITY_CENTER:
        center = height / 2f;
        TypedValue outValue = new TypedValue();
        getResources().getValue(R.dimen.biography_stl_divider_thickness, outValue, true);
        float value = outValue.getFloat();
        top = center - (thickness / value);
        bottom = center + (thickness / value);
        break;
      case GRAVITY_BOTTOM:
      default:
  ...
  private void drawOverline(Canvas canvas, int right) {
    if (topBorderThickness <= 0) {
      return;
    }
    // Thin overline along the entire top edge
    borderPaint.setColor(topBorderColor);
    canvas.drawRect(0, 0, right, topBorderThickness, borderPaint);
  }

  private void drawUnderline(Canvas canvas, int right, int height) {
    if (bottomBorderThickness <= 0) {
      return;
    }
    // Thin underline along the entire bottom edge
    borderPaint.setColor(bottomBorderColor);
    canvas.drawRect(0, height - bottomBorderThickness, right, height, borderPaint);
  }
