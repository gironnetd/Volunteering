package com.sc.fr.confucianisme.layers.mvp.common.customs.viewpagers.fragmentpageradapter;
...
public class FavoritesNativePagerAdapter extends PagerAdapter implements ContentTextViewNativeBiography.Callbacks, PersonalLinearLayout.Callbacks {
  ...
  private final FavoritesPresenterInterface presenter;
  ...
  public FavoritesNativePagerAdapter(Activity activity,PageModel[] mPageModels, FavoritesPresenterInterface presenter) {
    //  this.mPageModels = mPageModels;
    this.activity = activity;
    this.mPageModels = new PageModel[presenter.getFavoritesPageModels().length];
  ...
  public int getCount() {
    ...
    return presenter.getFavoritesPageModels().length;
  }
  ...
  public Object instantiateItem(ViewGroup container, int position) {
    ...
    PageModel currentPage = presenter.getFavoritesPageModels()[position];
    ...
    if( !tpString.trim().equals(""))
      ...
      typeface = Typeface.createFromAsset(OnelittleAngelApplication.instance.getAssets(), "fonts/mtcorsva.ttf");
...
//            else aNative.setBackgroundColor(Color.GREEN);
            ...
            aNative.setIncludeFontPadding(false);
            aNative.setText(s);
            aNative.setCallbacks(FavoritesNativePagerAdapter.this);
...
//        else currentPage.contentTextViewNatives.get(i).setBackgroundColor(Color.GRAY);
    ...
    if (position != 1 && presenter.getFavoritesPageModels().length != 1) {
      animate(llTxtView).alpha(0);
      animate(cardView).alpha(0);
    }

    mPageModels[position] = currentPage;
