package com.sc.en.onelittleangel.layers.dao.themes;

import com.sc.en.onelittleangel.layers.dao.DaoManager;
...
public class ThemesDao implements ThemesDaoInterface {
  ...
  public List<Theme> findThemesByIdParent(int idParent) {
    realm = Realm.getDefaultInstance();
    if(idParent == 0)
      return realm.where(Theme.class).isNull("parentTheme").findAll().sort("theme");
    else
      return realm.where(Theme.class).equalTo("parentTheme.idTheme",idParent).findAll().sort("theme");
  }

  @Override
  public List<Theme> findAllThemes() {
    realm = Realm.getDefaultInstance();
    return realm.where(Theme.class).findAll().sort("theme");
  }
  ...
  public List<Theme> findThemesWithThemes() {
    realm = Realm.getDefaultInstance();
    List<Theme> themes =  realm.where(Theme.class).isNull("parentTheme").findAll().sort("theme");
