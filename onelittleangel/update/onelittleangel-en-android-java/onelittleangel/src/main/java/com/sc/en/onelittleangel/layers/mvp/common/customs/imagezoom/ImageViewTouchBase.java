package com.sc.en.onelittleangel.layers.mvp.common.customs.imagezoom;
...
import com.sc.en.onelittleangel.layers.mvp.common.customs.imagezoom.graphics.FastBitmapDrawable;
import com.sc.en.onelittleangel.layers.mvp.common.customs.imagezoom.utils.IDisposable;
...
public abstract class ImageViewTouchBase extends android.support.v7.widget.AppCompatImageView implements IDisposable {
    ...
    public void setImageBitmap(final Bitmap bitmap) {
        setImageBitmap(bitmap, ZOOM_INVALID, ZOOM_INVALID);
    }
     ...
     */
    public void setImageBitmap(final Bitmap bitmap, float minZoom, float maxZoom) {
        if (bitmap != null) {
            setImageDrawable(new FastBitmapDrawable(bitmap), null, minZoom, maxZoom);
        } else {
            setImageDrawable(null, null, minZoom, maxZoom);
        }
    }
    ...
    private void setImageDrawable(final Drawable drawable, final Matrix initialMatrix, final float minZoom, final float maxZoom) {
        ...
        if (viewWidth <= 0) {
            mLayoutRunnable = () -> setImageDrawable(drawable, initialMatrix, minZoom, maxZoom);
            return;
        }
...
//                if(width > height) {
//                    if(OnelittleAngelApplication.instance.getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
//                        deltaY = (mViewPort.height() - height) / 3 - (rect.top - mViewPort.top);
//                    } else {
//                        deltaY = (mViewPort.height() - height) / 5 - (rect.top - mViewPort.top);
//                    }
//                } else {
//                    deltaY = (mViewPort.height() - height) / 5 - (rect.top - mViewPort.top);
//                }
...
//        }
        ...
        ValueAnimator animation = ValueAnimator.ofFloat(oldScale, scale);
