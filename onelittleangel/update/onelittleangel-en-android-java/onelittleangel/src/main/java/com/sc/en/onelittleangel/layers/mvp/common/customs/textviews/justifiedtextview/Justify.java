...
package com.sc.en.onelittleangel.layers.mvp.common.customs.textviews.justifiedtextview;
...
class Justify {
  ...
  static final float DEFAULT_MAX_PROPORTION = 20f;
  ...
  static void setupScaleSpans(final @NotNull Justified justified,
    ...
    final CharSequence text = /*textView.getText().toString().contains("\n\n") ? textView.getText().toString().replace("\n\n", "\n") :*/ textView.getText();
  ...
  //  Log.v("test2", "textView.getCompoundPaddingLeft() : " + textView.getCompoundPaddingLeft());
  //  Log.v("test2", "textView.getCompoundPaddingRight() : " + textView.getCompoundPaddingRight());
...
//      lineCount = count;
    ...
    //  if (spannable.charAt(visibleLineEnd - 1) == '\n') continue;

      if (line == count - 1 || spannable.charAt(lineEnd - 1) == '\n') {
//        if(line == count - 1) Log.v("test3", "line == count - 1");
//        if(spannable.charAt(lineEnd - 1) == '\n'){
//          Log.v("test3", "spannable.charAt(lineEnd - 1) == '\\n'");
//
//        }
      //  Log.v("test2", "line == count");
        lastLine = spannable.subSequence(lineStart, lineEnd).toString();
        lastLinePosition = lineStart;
        lineCount = count;
        //Log.v("test3", " : " + lineCount);
        //  Log.v("test2", "lastline : " + spannable.subSequence(lastLinePosition, lineEnd).toString());
        continue;
      } else {
      //  Log.v("test2", "line != count");

        //  Log.v("test2", "line != count : " + (textView.getLayout().getLineBottom(line) - textView.getLayout().getLineTop(line)));
      //  Log.v("test2", "textView.getLineSpacingExtra() : " + textView.getLineSpacingExtra());
      //  Log.v("test2", "textView.getLineSpacingMultiplier() : " + textView.getLineSpacingMultiplier());

      //  Log.v("test2", "textView.getCompoundPaddingBottom() : " + textView.getCompoundPaddingBottom());
      //  Log.v("test2", "textView.getCompoundPaddingTop() : " + textView.getCompoundPaddingTop());

      //  Log.v("test2", "textView.getLineHeight() : " + textView.getLineHeight());
      //  Log.v("test2", "textView.getMeasuredHeight() : " + textView.getMeasuredHeight());
      //  lastLine = "";
      //  lastLinePosition = 0;
      //  lineCount = 0;
      //  Log.v("test2", "line : " + spannable.subSequence(lineStart, visibleLineEnd).toString());
      }

      // Don't justify lines that only contain whitespace
      if (visibleLineEnd == lineStart){
      //  Log.v("test2", "Don't justify lines that only contain whitespace ");
        continue;
      }

      // Layout line width
      final float w3 = layout.getLineWidth(line);    // Works fine, but only for API > 11
    //  Log.v("test2", "w3 Works fine, but only for API > 11 : " + w3);

      final float w2 = layout.getLineMax(line);      // Doesn't work well
    //  Log.v("test2", "w2 Doesn't work well : " + w2);
    ...
    //  Log.v("test2", "remaining : " + remaining);
    //  Log.v("test2", "length : " + length);
    //  Log.v("test2", "layout.getLineEnd(line) : " + layout.getLineEnd(line));
      //Log.v("test2", "length : " + spannable.subSequence(remaining, visibleLineEnd));
      ...
      if (remaining > 0) {
        ...
        //Log.v("test2", " sub : " + spannable.subSequence(lineStart, visibleLineEnd));
        ...
        //  Log.v("test2", " test : " + spannable.subSequence(lineStart + matchStart, lineStart + matchEnd));
        ...
        //  Log.v("test2", "n > textViewSpans.length");
        ...
        for (int span=0; span<n; ++span) {
          ...
          spannable.setSpan(
            ...
            Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
        //  Log.v("test2", "textViewSpans " + n + " : " + textViewSpans.);
  ...
  public static int getLastLinePosition(){
  ...
  public static int getRealLineCount(){
    return lineCount;
  }
