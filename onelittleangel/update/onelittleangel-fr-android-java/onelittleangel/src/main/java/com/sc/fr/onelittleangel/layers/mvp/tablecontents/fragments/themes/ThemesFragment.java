package com.sc.fr.onelittleangel.layers.mvp.tablecontents.fragments.themes;
...
import com.sc.fr.onelittleangel.layers.mvp.tablecontents.fragments.BaseFragment;
...
import com.sc.fr.onelittleangel.layers.mvp.tablecontents.models.Theme;
import com.sc.fr.onelittleangel.layers.mvp.tablecontents.models.base.adapters.listadapters.BaseListAdapter;
import com.sc.fr.onelittleangel.layers.mvp.tablecontents.models.base.views.listviews.BaseListView;
...
public class ThemesFragment extends BaseFragment implements ViewInterface {
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    if(presenter == null) presenter = PresenterInjector.getThemesPresenter(this);
  ...
  protected void onCreateViewAfterViewStubInflated(View inflatedView, Bundle savedInstanceState) {
    ...
    if (inflatedView != null) {
      ...
      baseListAdapter = new BaseListAdapter<>(getActivity(), presenter, type, themesList);
  ...
  private void resizeView() {
    ...
    if (presenter.getMovements() == null || presenter.getMovements().size() == 0) {
      ...
      editor.putInt(Constants.TABLE_CONTENTS_CURRENT_POSITION, 3).apply();
    }
    else cvNoContents.setVisibility(View.GONE);

    String[] openPositionArray = settings.getString(getResources().getString(R.string.themes) ,"").split(" - ");

    if (!settings.getString(getResources().getString(R.string.themes), "").equals("-1") &&
            !settings.getString(getResources().getString(R.string.themes), "").equals("") && presenter.getMovements().size() != 0) {
    ...
    if (((TableContentsActivity) getActivity()).viewPager.getMeasuredHeight() != heightToApply) {
      ...
      if (heightToApply < height - TableContentsActivity.headerHeight + OnelittleAngelApplication.instance.getResources().getDimension(R.dimen.activity_horizontal_margin)) heightToApply = (int) (height - TableContentsActivity .headerHeight + OnelittleAngelApplication.instance.getResources().getDimension(R.dimen.activity_horizontal_margin));
      ...
      if (!settings.getString(getResources().getString(R.string.themes), "").equals("-1") &&
              !settings.getString(getResources().getString(R.string.themes), "").equals("")) {
    ...
    if(TableContentsActivity.isOrientationChanged || TableContentsActivity.navigationIdOpened || TableContentsActivity.isFromOtherActivity) {
      ...
      if (3 < settings.getInt(Constants.TABLE_CONTENTS_CURRENT_POSITION, 0)) setLeftAdapter();
      else if (3 > settings.getInt(Constants.TABLE_CONTENTS_CURRENT_POSITION, 0)) setRightAdapter();
      else setScaleAdapter();
    }

    TableContentsActivity.fromFragment = getResources().getString(R.string.themes);
    editor.putInt(Constants.TABLE_CONTENTS_CURRENT_POSITION, 3);
    //  editor.commit();
    editor.apply();
