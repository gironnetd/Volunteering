package com.sc.fr.onelittleangel.layers.mvp.tablecontents.fragments.authors;
...
public class AuthorsFragment extends BaseFragment implements ViewInterface {

  /***********************************************************
   *  Attributes
   **********************************************************/

  private List<AuthorBook> authorsList = new ArrayList<>();
  ...
  public static AuthorsFragment newInstance() {
    ...
    args.putString(TYPE, "authors");
    authorsFragment.setArguments(args);
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    if(presenter == null) {presenter = PresenterInjector.getAuthorsPresenter(this);
    }
  }
  ...
  protected void onCreateViewAfterViewStubInflated(View inflatedView, Bundle savedInstanceState) {
    ...
    if (inflatedView != null) {
      ...
      //llViewStubFragment = (LinearLayout) inflatedView.findViewById(R.id.ll_view_stub_fragment);
      ...
      baseListAdapter = new BaseListAdapter<>(getActivity(), presenter, type, authorsList);
  ...
  private void resizeView() {
    ...
    if (presenter.getMovements() == null || presenter.getMovements().size() == 0) {
      ...
      editor.putInt(Constants.TABLE_CONTENTS_CURRENT_POSITION, 1).apply();

    }
    else cvNoContents.setVisibility(View.GONE);

    String[] openPositionArray = settings.getString(getResources().getString(R.string.authors) ,"").split(" - ");

    if (!settings.getString(getResources().getString(R.string.authors), "").equals("-1") &&
            !settings.getString(getResources().getString(R.string.authors), "").equals("") && presenter.getMovements().size() != 0) {
      ...
      if (presenter.getMovements().size() == 1) {
        if (presenter.getMovements().get(0).baseEntities != null) {
          heightToApply = (int) (presenter.getMovements().size() * settings.getInt(CardViewNative.MCARDVIEWMEASUREDHEIGHT, 0) + TableContentsActivity.headerHeight + (settings.getInt(CardViewNative.MCARDVIEWMEASUREDHEIGHT, 0) + (settings.getInt(CardViewNative.MCARDVIEWMEASUREDHEIGHT, 0) * presenter.getMovements().get(0).baseEntities.size())) + OnelittleAngelApplication.instance.getResources().getDimension(R.dimen.activity_horizontal_margin));
    ...
    if (((TableContentsActivity) getActivity()).viewPager.getMeasuredHeight() != heightToApply) {
      ...
      if (!settings.getString(getResources().getString(R.string.authors), "").equals("-1") &&
              !settings.getString(getResources().getString(R.string.authors), "").equals("")) {
    ...
    if(TableContentsActivity.isOrientationChanged || TableContentsActivity.navigationIdOpened || TableContentsActivity.isFromOtherActivity) {
      ...
      else if (1 > settings.getInt(Constants.TABLE_CONTENTS_CURRENT_POSITION, 0)) setRightAdapter();
      else setScaleAdapter();
    }
    TableContentsActivity.fromFragment = getResources().getString(R.string.authors);
    editor.putInt(Constants.TABLE_CONTENTS_CURRENT_POSITION, 1);
    //  editor.commit();
    editor.apply();
  }
