package com.sc.fr.onelittleangel.layers.mvp.tablecontents.fragments.movements;
...
import com.sc.fr.onelittleangel.layers.mvp.tablecontents.models.Faith;
...
public class MovementsFragment extends BaseFragment implements ViewInterface {

  /***********************************************************
   *  Attributes
   **********************************************************/

  private List<Faith> faithsList = new ArrayList<>();
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    if(presenter == null) presenter = PresenterInjector.getMovementsPresenter(this);
  ...
  protected void onCreateViewAfterViewStubInflated(View inflatedView, Bundle savedInstanceState) {
    ...
    if (inflatedView != null) {
      ...
      baseListAdapter = new BaseListAdapter<>(getActivity(), presenter, type, faithsList);
  ...
  /***********************************************************
   *  Implement the MovementsTBViewInterface
   **********************************************************/
  ...
  private void resizeView() {
    ...
    if (presenter.getMovements() == null || presenter.getMovements().size() == 0) {
      ...
      editor.putInt(Constants.TABLE_CONTENTS_CURRENT_POSITION, 2).apply();

    }
    else cvNoContents.setVisibility(View.GONE);

    String[] openPositionArray = settings.getString(getResources().getString(R.string.movements) ,"").split(" - ");

    if (!settings.getString(getResources().getString(R.string.movements), "").equals("-1") &&
            !settings.getString(getResources().getString(R.string.movements), "").equals("") && presenter.getMovements().size() != 0) {
      ...
      if (openPositionArray.length != 0 && openPositionArray.length == 1) {
        ...
        } else if (presenter.getMovements().get(Integer.parseInt(openPositionArray[0])).authorBooks != null) {
    ...
    if (((TableContentsActivity) getActivity()).viewPager.getMeasuredHeight() != heightToApply) {
      ...
      if (!settings.getString(getResources().getString(R.string.movements), "").equals("-1") &&
              !settings.getString(getResources().getString(R.string.movements), "").equals("")) {
    ...
    if(TableContentsActivity.isOrientationChanged || TableContentsActivity.navigationIdOpened || TableContentsActivity.isFromOtherActivity) {
      ...
      editor.putBoolean(Constants.IS_ANIMATING_LISTVIEW_ENABLED, true).commit();
      if (2 < settings.getInt(Constants.TABLE_CONTENTS_CURRENT_POSITION, 0)) setLeftAdapter();
      else if (2 > settings.getInt(Constants.TABLE_CONTENTS_CURRENT_POSITION, 0)) setRightAdapter();
      else setScaleAdapter();
    }
    TableContentsActivity.fromFragment = getResources().getString(R.string.movements);
    editor.putInt(Constants.TABLE_CONTENTS_CURRENT_POSITION, 2);
//editor.commit();
    editor.apply();
