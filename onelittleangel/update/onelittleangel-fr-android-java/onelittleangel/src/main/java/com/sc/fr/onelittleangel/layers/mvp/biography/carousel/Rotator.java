...
class Rotator {
     ...
     */
    void startRotate(float dAngleDeg, int durationMillis) {
        mMode = SCROLL_MODE;
        mFinished = false;
    ...
    void fling(float velocityAngle) {
        ...
        mDurationMillis = (int) (250.0f * Math.sqrt(2.0f * mCoeffVelocity * Math.abs(velocityAngle)
                / DECELERATION));
